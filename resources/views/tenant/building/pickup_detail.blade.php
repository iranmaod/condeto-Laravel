...
<div class="main_content">
    ...
    <div class="container repairs_details message_detail " data-type="message_event" data-object_id="324" data-page_count="1" data-per_page="5">
        ...
        <h2><img src="{{asset('img/event.png')}}" /> {{$pickup->description}} - {{ date('d F Y', strtotime($pickup->create_time))  }} - Pickup Request
            </h2>
 ...
 @endforeach
    ...
    -->
        ...
        <div class="">
            <form style="display:none;" id="theMessage" action="{{route('mybuilding.mypickupsnewnote',$pickup->id)}}" method="post">
            @csrf
                <h3>Add New Note</h3>
                <div class="row">
                    <div class="field">
        ...
        <form action="{{route('mybuilding.mypickupsUpdate',$pickup->id)}}" method="post" >
            @csrf
            <input type="hidden" name="event_id" id="event_id" value="{{$pickup->id}}" />
            ...
            <div class="row">
                <div class="field">				
                    <select name="status_of_task" id="status_of_task">					
                        <option value="0" {{ $pickup->status == 0 ? 'selected' : '' }}>In Process</option>
                        <option value="1" {{ $pickup->status == 1 ? 'selected' : '' }}>Completed</option>					
                    </select>
                </div>
            </div>
